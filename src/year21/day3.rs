...
fn find_common<'a>(lines: &Vec<&'a str>, nth_bit: usize, ones: bool) -> (Vec<&'a str>, Vec<&'a str>) {
    let num_lines = lines.len() as u32;
    let count_ones = count_ones(lines);

    let build_str = |x: &u32| match ones {
        true => { if num_lines - *x > *x {"0"} else {"1"}},
        false => { if num_lines - *x > *x {"1"} else {"0"}},
    };

    let most_common: Vec<&str> = count_ones.iter().map(build_str).collect();

    let lines_left = lines.clone().into_iter().filter(|x| x.chars().nth(nth_bit).unwrap().to_string().eq(most_common[nth_bit])).collect();
