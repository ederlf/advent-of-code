...
fn create_board(board_spec:&[&str]) -> Board {
    let mut board = Board{values: HashMap::new(), rows: [0,0, 0,0,0], columns: [0,0, 0,0,0]};
    for (x, row) in board_spec.iter().enumerate() {
        for (y, value) in row.split(" ").filter(|x| !x.is_empty()).enumerate() {
            let value = value.parse::<u32>().unwrap();
            let point = Point{x: x as u32, y: y as u32, marked: false, value};
    ...
    for value in values {
        for (i, board) in boards.iter_mut().enumerate() {
            ...
            if win > -1  {
                winners.insert(i);
                if first {
                    return win as u32 * board.values
                                        .values()
                                        .filter(|x| !x.marked)
...
fn part1(input: String)  -> String {
    ...
    let values = lines[0].split(",").filter(|x| !x.is_empty()).map(|x| x.parse::<u32>().unwrap()).collect();
